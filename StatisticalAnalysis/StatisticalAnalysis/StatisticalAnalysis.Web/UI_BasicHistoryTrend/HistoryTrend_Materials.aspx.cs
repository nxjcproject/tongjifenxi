...
{
    public partial class MaterialsQuantityTrend : WebStyleBaseForEnergy.webStyleBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                this.OrganisationTree_ProductionLine.PageName = "HistoryTrend_Materials.aspx";                                     //向web用户控件传递当前调用的页面名称
        ...
        private static DataTable GetQuantityUsage(DataTable tagTable, string startTime, string endTime, string timeInterval)
        {
            ...
            DataTable QuantityUsageSumTable = MaterialsHistoryTrendService.GetQuantityUsageGroup(tagTable, DateTime.Parse(startTime), DateTime.Parse(endTime), timeInterval);
        ...
        ////////////////////////////弹出对话框物料列表//////////////////////////////
        [WebMethod]
        public static string GetMaterialsTreeGridFormat(string organizationId)
        {
            DataTable dt = StatisticalAnalysis.Service.BasicHistoryTrend.MaterialsHistoryTrendService.GetMaterialsByOrganizationID(organizationId);
            return EasyUIJsonParser.TreeGridJsonParser.DataTableToJson(dt, "KeyId", "Name", "ParentKeyId", "0", new string[] { "OrganizationId", "TagTableName", "TagColumnName", "Name", "LevelType", "VariableId" });
        }
    }
}