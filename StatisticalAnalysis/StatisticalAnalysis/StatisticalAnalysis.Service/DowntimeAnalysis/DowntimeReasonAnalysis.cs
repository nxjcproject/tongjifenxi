...
namespace StatisticalAnalysis.Service.DowntimeAnalysis
{
    public class DowntimeReasonAnalysis
    {
        ...
        public static DataTable GetHaltReasonStatics(string myStartTime, string myEndTime, string myEquipmentCommonId, string myStaticsMethod, string myLevelCode, string myHaltTypeId, string myReasonTypeId, List<string> myValidOrganizationIds)
        {
            DataTable m_HaltReasonInfoTable = GetHaltReasonInfo();
            string m_OrganizationIds = "";
            string m_Sql = @"Select X.ReasonText {7} as ReasonText, X.MachineHaltReasonID as MachineHaltReasonId, X.Value from
                               (Select M.MachineHaltReasonID, M.ReasonText, M.ReasonStatisticsTypeId, {6} 
                                 from
								    ...
								    from shift_MachineHaltLog C
                                        ...
                                        {5}) M
									group by M.ReasonStatisticsTypeId, M.MachineHaltReasonID, M.ReasonText) X
                                    {4}
            ...
            try
            {
                if (myValidOrganizationIds != null)
                {
                    ...
                    string m_ReasonStatisticsTypeName = "";
                    if (myHaltTypeId == "null")
                    {
                        m_ReasonStatisticsType = " where X.MachineHaltReasonID = '' ";
                    }
                    else if (myReasonTypeId == "All")
                    {
                        ...
                        m_ReasonStatisticsTypeName = "+ case when Y.Name is not null and Y.Name <> '' then '(' + Y.Name + ')' else '' end "; 
                    }
                    else
                    {
                    ...
                    m_Sql = m_Sql.Replace("{7}", m_ReasonStatisticsTypeName);
