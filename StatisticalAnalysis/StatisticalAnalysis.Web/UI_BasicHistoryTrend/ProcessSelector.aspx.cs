...
namespace StatisticalAnalysis.Web.UI_BasicHistoryTrend
{
    public partial class ProcessSelector : WebStyleBaseForEnergy.webStyleBase
    {
        ...
        {
            base.InitComponts();
            this.HiddenField_endTime.Value = Request.QueryString["endTime"].ToString();
            if (!IsPostBack)
            { 
                ////////////////////调试用,自定义的数据授权
#if DEBUG
                ...
                this.OrganisationTree_ProductionLine.PageName = "ComprehensiveAnalysis.aspx";   
                //向web用户控件传递当前调用的页面名称
            }
        }

        [WebMethod]
        public static string GetProcessWithTreeGridFormat(string organizationId, string mEndTime)
        {
            DataTable dt = ProcessService.GetSystemProcessByOrganizationID(organizationId, mEndTime);
